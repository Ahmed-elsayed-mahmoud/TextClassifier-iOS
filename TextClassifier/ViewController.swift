...
class ViewController: UIViewController {
    ...
    @IBAction func speechToText(_ sender: Any) {
        VoiceController.shared.confirmResult(message: "Please start speaking after changing the state to 'Listening'", viewController: self) { result in
            if !(result ?? "").isEmpty {
                self.inputTextView.text = result
                self.classifyButtonTouchUpInside(self)
            } else if !(self.inputTextView.text ?? "").isEmpty {
                VoiceController.shared.speakFewWords(message: "The sentiment classification is '\(self.categories.first?.categoryName ?? "")' and the inference time is \(self.inferenceTimeLabel.text ?? "")", viewController: self)
            }
        }
    }
    ...
    private func setupUI() {
        ...
        VoiceController.shared.enablePermissions {
